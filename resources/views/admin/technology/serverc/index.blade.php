...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div><h1 class="text-2xl font-bold text-gray-800">Server & Cloud</h1><p class="text-sm text-gray-500 mt-0.5">Kelola data server fisik dan cloud</p></div>
    <button onclick="openModal()" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2.5 rounded-xl shadow-sm transition-colors"><i class="fa fa-plus"></i> Tambah Server</button>
</div>
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        <div class="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center text-blue-500 text-lg flex-shrink-0"><i class="fa fa-server"></i></div>
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalServer }}</p><p class="text-xs text-gray-500 mt-1">Total Server</p></div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        ...
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalDown }}</p><p class="text-xs text-gray-500 mt-1">Nonaktif</p></div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        <div class="w-12 h-12 rounded-xl bg-purple-50 flex items-center justify-center text-purple-500 text-lg flex-shrink-0"><i class="fa fa-cloud-arrow-up"></i></div>
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalBackup }}</p><p class="text-xs text-gray-500 mt-1">Backup Aktif</p></div>
    </div>
</div>
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-gray-100">
        <div><h2 class="font-semibold text-gray-800 text-base">Daftar Server</h2><p class="text-xs text-gray-400 mt-0.5">{{ $data->count() }} total</p></div>
        <div class="relative"><i class="fa fa-search absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
            <input type="text" placeholder="Cari server..." oninput="onSearch(this.value)" class="pl-8 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 w-44"></div>
    </div>
    <div class="flex flex-wrap items-center gap-3 px-5 py-3 border-b border-gray-100 text-xs">        <select id="filterStatus" onchange="filterTable()" class="border border-gray-200 rounded-lg px-2 py-1 text-xs focus:outline-none">
            <option value="">Semua Status</option><option>Aktif</option><option>Nonaktif</option><option>Maintenance</option></select>
    </div>
    <div class="overflow-x-auto"><table class="w-full text-sm">
        <thead><tr class="bg-gray-50 border-b border-gray-100">
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">No</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Nama Server</th>
            ...
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">OS</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Provider Cloud</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Status</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Backup Aktif</th>
            <th class="text-center text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Aksi</th>
        </tr></thead>
        <tbody id="tableBody">
            @forelse($data as $d)
            <tr class="border-t border-gray-50 odd:bg-white even:bg-gray-100 hover:bg-blue-50/50 transition-colors"
                data-search="{{ strtolower($d->nama_server.' '.$d->jenis_server.' '.$d->lokasi.' '.$d->os) }}"
                data-status="{{ $d->status }}">
                <td class="px-4 py-3.5 text-gray-400">{{ $data->firstItem() + $loop->index }}</td>
                <td class="px-4 py-3.5 text-sm font-semibold text-gray-800">{{ $d->nama_server }}</td>
                <td class="px-4 py-3.5 text-sm text-gray-600">{{ $d->jenis_server }}</td>
                <td class="px-4 py-3.5 text-sm text-gray-600">{{ $d->lokasi }}</td>
                <td class="px-4 py-3.5 text-sm text-gray-600">{{ $d->os }}</td>
                <td class="px-4 py-3.5 text-sm text-gray-500">{{ $d->provider_cloud ?? '-' }}</td>
                <td class="px-4 py-3.5">
                    @php $sc=['Aktif'=>'green','Nonaktif'=>'red','Maintenance'=>'yellow'][$d->status]??'gray' @endphp
                ...
                <td class="px-4 py-3.5">
                    @if($d->backup_aktif)<span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-600">Ya</span>
                    @else<span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-500">Tidak</span>@endif
                </td>
                <td class="px-4 py-3.5"><div class="flex items-center justify-center gap-1.5">
                    <button onclick="triggerEdit(this)" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-yellow-100 text-yellow-600 hover:bg-yellow-200 transition-colors"
                        data-action="{{ route('serverc.update', $d->id) }}"
                        data-nama_server="{{ $d->nama_server }}" data-jenis_server="{{ $d->jenis_server }}"
                        data-lokasi="{{ $d->lokasi }}" data-os="{{ $d->os }}"
                        data-provider_cloud="{{ $d->provider_cloud }}" data-status="{{ $d->status }}"
                        data-backup_aktif="{{ $d->backup_aktif ? '1' : '0' }}">
                    ...
                    <button onclick="triggerDelete(this)" type="button" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-red-100 text-red-600 hover:bg-red-200 transition-colors"
                        data-action="{{ route('serverc.destroy', $d->id) }}" data-name="{{ $d->nama_server }}">
            ...
            <tr><td colspan="9" class="px-5 py-12 text-center"><div class="flex flex-col items-center gap-3">
                <div class="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center"><i class="fa fa-server text-2xl text-gray-300"></i></div>
                <p class="text-sm font-medium text-gray-500">Belum ada data server</p></div></td></tr>
            @endforelse
        </tbody>
    </table>
...
<div id="mainModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-xl mx-4 max-h-[90vh] overflow-y-auto" style="animation:slideUp .2s ease">
        <div class="flex items-start justify-between px-6 py-5 border-b border-gray-100 sticky top-0 bg-white">
            <h2 id="modalTitle" class="text-base font-bold text-gray-800">Tambah Server</h2>
            <button onclick="closeModal()" class="text-gray-400 hover:text-red-500 text-lg leading-none"><i class="fa fa-times"></i></button>
        </div>
        <form id="mainForm" action="{{ route('serverc.store') }}" method="POST" class="px-6 py-5 space-y-4">
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4" style="animation:slideUp .2s ease">
        <div class="px-6 pt-6 pb-2 text-center">
            ...
            <h2 class="text-base font-bold text-gray-800 mt-4">Hapus Server?</h2>
...
const createUrl="{{ route('serverc.store') }}";
function openModal(){document.getElementById('modalTitle').innerText='Tambah Server';mainForm.action=createUrl;methodContainer.innerHTML='';mainForm.reset();mainModal.classList.remove('hidden');mainModal.classList.add('flex');}
...
function triggerEdit(btn){document.getElementById('modalTitle').innerText='Edit Server';mainForm.action=btn.dataset.action;methodContainer.innerHTML='<input type="hidden" name="_method" value="PUT">';
['nama_server','jenis_server','lokasi','os','provider_cloud','status','backup_aktif'].forEach(k=>{const el=document.getElementById('f_'+k);if(el)el.value=btn.dataset[k]??'';});mainModal.classList.remove('hidden');mainModal.classList.add('flex');}
