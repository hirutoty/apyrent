...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div><h1 class="text-2xl font-bold text-gray-800">Policy & Compliance</h1><p class="text-sm text-gray-500 mt-0.5">Kelola kebijakan dan kepatuhan IT</p></div>
    <button onclick="openModal()" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2.5 rounded-xl shadow-sm transition-colors"><i class="fa fa-plus"></i> Tambah Dokumen</button>
</div>
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        <div class="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center text-blue-500 text-lg flex-shrink-0"><i class="fa fa-file-shield"></i></div>
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalDokumen }}</p><p class="text-xs text-gray-500 mt-1">Total Dokumen</p></div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        <div class="w-12 h-12 rounded-xl bg-yellow-50 flex items-center justify-center text-yellow-500 text-lg flex-shrink-0"><i class="fa fa-pen"></i></div>
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalDraft }}</p><p class="text-xs text-gray-500 mt-1">Draft</p></div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        <div class="w-12 h-12 rounded-xl bg-orange-50 flex items-center justify-center text-orange-500 text-lg flex-shrink-0"><i class="fa fa-magnifying-glass"></i></div>
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalReview }}</p><p class="text-xs text-gray-500 mt-1">Review</p></div>
    </div>
</div>
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-gray-100">
        <div><h2 class="font-semibold text-gray-800 text-base">Daftar Dokumen</h2><p class="text-xs text-gray-400 mt-0.5">{{ $data->count() }} total</p></div>
        <div class="relative"><i class="fa fa-search absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
            <input type="text" placeholder="Cari dokumen..." oninput="onSearch(this.value)" class="pl-8 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 w-44"></div>
    </div>
    <div class="flex flex-wrap items-center gap-3 px-5 py-3 border-b border-gray-100 text-xs">
        ...
        <select id="filterStatus" onchange="renderTable()" class="border border-gray-200 rounded-lg px-2 py-1 text-xs focus:outline-none">
            <option value="">Semua Status</option><option>Aktif</option><option>Draft</option><option>Review</option><option>Archived</option></select>
    </div>
    <div class="overflow-x-auto"><table class="w-full text-sm">
        <thead><tr class="bg-gray-50 border-b border-gray-100">
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">No</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Nama Dokumen</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Versi</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Tanggal Berlaku</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Tanggung Jawab</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Status</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Sertifikasi Terkait</th>
            <th class="text-center text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Aksi</th>
        </tr></thead>
        <tbody id="tableBody">
            @forelse($data as $d)
            <tr class="border-t border-gray-50 odd:bg-white even:bg-gray-100 hover:bg-blue-50/50 transition-colors"
                data-search="{{ strtolower($d->nama_dokumen.' '.$d->tanggung_jawab.' '.$d->sertifikasi_terkait) }}"
                data-status="{{ $d->status }}">
                <td class="px-4 py-3.5 text-gray-400">{{ $data->firstItem() + $loop->index }}</td>
                <td class="px-4 py-3.5 text-sm font-semibold text-gray-800">{{ $d->nama_dokumen }}</td>
                <td class="px-4 py-3.5"><span class="font-mono text-xs bg-gray-100 px-2 py-0.5 rounded">{{ $d->versi }}</span></td>
                <td class="px-4 py-3.5 text-sm text-gray-500">{{ $d->tanggal_berlaku ? \Carbon\Carbon::parse($d->tanggal_berlaku)->format('d/m/Y') : '-' }}</td>
                <td class="px-4 py-3.5 text-sm text-gray-600">{{ $d->tanggung_jawab }}</td>
                <td class="px-4 py-3.5">
                    @php $sc=['Aktif'=>'green','Draft'=>'yellow','Review'=>'orange','Archived'=>'gray'][$d->status]??'gray' @endphp
                ...
                <td class="px-4 py-3.5 text-sm text-gray-500">{{ $d->sertifikasi_terkait ?? '-' }}</td>
                <td class="px-4 py-3.5"><div class="flex items-center justify-center gap-1.5">
                    <button onclick="triggerEdit(this)" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-yellow-100 text-yellow-600 hover:bg-yellow-200 transition-colors"
                        data-action="{{ route('policyc.update', $d->id) }}"
                        data-nama_dokumen="{{ $d->nama_dokumen }}" data-versi="{{ $d->versi }}"
                        data-tanggal_berlaku="{{ $d->tanggal_berlaku }}" data-tanggung_jawab="{{ $d->tanggung_jawab }}"
                        data-status="{{ $d->status }}" data-sertifikasi_terkait="{{ $d->sertifikasi_terkait }}">
                    ...
                    <button onclick="triggerDelete(this)" type="button" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-red-100 text-red-600 hover:bg-red-200 transition-colors"
                        data-action="{{ route('policyc.destroy', $d->id) }}" data-name="{{ $d->nama_dokumen }}">
            ...
            <tr><td colspan="8" class="px-5 py-12 text-center"><div class="flex flex-col items-center gap-3">
                <div class="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center"><i class="fa fa-file-shield text-2xl text-gray-300"></i></div>
                <p class="text-sm font-medium text-gray-500">Belum ada data kebijakan</p></div></td></tr>
            @endforelse
        </tbody>
    </table>
...
<div id="mainModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-xl mx-4 max-h-[90vh] overflow-y-auto" style="animation:slideUp .2s ease">
        <div class="flex items-start justify-between px-6 py-5 border-b border-gray-100 sticky top-0 bg-white">
            <h2 id="modalTitle" class="text-base font-bold text-gray-800">Tambah Dokumen</h2>
            <button onclick="closeModal()" class="text-gray-400 hover:text-red-500 text-lg leading-none"><i class="fa fa-times"></i></button>
        </div>
        <form id="mainForm" action="{{ route('policyc.store') }}" method="POST" class="px-6 py-5 space-y-4">
            @csrf <div id="methodContainer"></div>
            <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Nama Dokumen <span class="text-red-500">*</span></label>
                <input type="text" name="nama_dokumen" id="f_nama_dokumen" required placeholder="Kebijakan Keamanan Informasi" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('nama_dokumen') }}"></div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Versi <span class="text-red-500">*</span></label>
                    <input type="text" name="versi" id="f_versi" required placeholder="v1.0" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('versi') }}"></div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Tanggung Jawab <span class="text-red-500">*</span></label>
                    <input type="text" name="tanggung_jawab" id="f_tanggung_jawab" required placeholder="IT Manager / Direktur" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('tanggung_jawab') }}"></div>
            ...
            <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Sertifikasi Terkait</label>
                <input type="text" name="sertifikasi_terkait" id="f_sertifikasi_terkait" placeholder="ISO 27001 / SOC 2 / PCI-DSS" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('sertifikasi_terkait') }}"></div>
...
const createUrl="{{ route('policyc.store') }}";
function openModal(){document.getElementById('modalTitle').innerText='Tambah Dokumen';mainForm.action=createUrl;methodContainer.innerHTML='';mainForm.reset();mainModal.classList.remove('hidden');mainModal.classList.add('flex');}
...
function triggerEdit(btn){document.getElementById('modalTitle').innerText='Edit Dokumen';mainForm.action=btn.dataset.action;methodContainer.innerHTML='<input type="hidden" name="_method" value="PUT">';
['nama_dokumen','versi','tanggal_berlaku','tanggung_jawab','status','sertifikasi_terkait'].forEach(k=>{const el=document.getElementById('f_'+k);if(el)el.value=btn.dataset[k]??'';});mainModal.classList.remove('hidden');mainModal.classList.add('flex');}
