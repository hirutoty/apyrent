...
@section('title', 'Project Management IT')
@section('content')
<div class="space-y-6">
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div><h1 class="text-2xl font-bold text-gray-800">Project Management IT</h1><p class="text-sm text-gray-500 mt-0.5">Kelola proyek teknologi informasi</p></div>
    <button onclick="openModal()" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2.5 rounded-xl shadow-sm transition-colors"><i class="fa fa-plus"></i> Tambah Proyek</button>
</div>
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        ...
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalProyek }}</p><p class="text-xs text-gray-500 mt-1">Total Proyek</p></div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        ...
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalDone }}</p><p class="text-xs text-gray-500 mt-1">Selesai</p></div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-gray-100">
        <div><h2 class="font-semibold text-gray-800 text-base">Daftar Proyek</h2><p class="text-xs text-gray-400 mt-0.5">{{ $data->count() }} total</p></div>
        <div class="relative"><i class="fa fa-search absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
            <input type="text" placeholder="Cari proyek..." oninput="onSearch(this.value)" class="pl-8 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 w-44"></div>
    </div>
    <div class="flex flex-wrap items-center gap-3 px-5 py-3 border-b border-gray-100 text-xs">        <select id="filterStatus" onchange="filterTable()" class="border border-gray-200 rounded-lg px-2 py-1 text-xs focus:outline-none">
            <option value="">Semua Status</option><option>In Progress</option><option>Selesai</option><option>Pending</option></select>
    </div>
    <div class="overflow-x-auto"><table class="w-full text-sm">
        <thead><tr class="bg-gray-50 border-b border-gray-100">
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">No</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Nama Proyek</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">PIC Proyek</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Tujuan</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Estimasi</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Status</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Progres</th>
            <th class="text-center text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Aksi</th>
        </tr></thead>
        <tbody id="tableBody">
            @forelse($data as $d)
            <tr class="border-t border-gray-50 odd:bg-white even:bg-gray-100 hover:bg-blue-50/50 transition-colors"
                data-search="{{ strtolower($d->nama_proyek.' '.$d->pic_proyek.' '.$d->tujuan) }}"
                data-status="{{ $d->status }}">
                <td class="px-4 py-3.5 text-gray-400">{{ $data->firstItem() + $loop->index }}</td>
                <td class="px-4 py-3.5 text-sm font-semibold text-gray-800">{{ $d->nama_proyek }}</td>
                <td class="px-4 py-3.5 text-sm text-gray-600">{{ $d->pic_proyek }}</td>
                <td class="px-4 py-3.5 text-sm text-gray-600 max-w-[180px] truncate">{{ $d->tujuan }}</td>
                <td class="px-4 py-3.5 text-sm text-gray-500">{{ $d->estimasi_waktu }}</td>
                <td class="px-4 py-3.5">
                    @php $sc=['In Progress'=>'yellow','Selesai'=>'green','Pending'=>'gray'][$d->status]??'blue' @endphp
                ...
                <td class="px-4 py-3.5">
                    <div class="flex items-center gap-2">
                        <div class="w-20 bg-gray-100 rounded-full h-1.5"><div class="bg-blue-500 h-1.5 rounded-full" style="width:{{ $d->progres }}%"></div></div>
                        <span class="text-xs text-gray-600">{{ $d->progres }}%</span>
                    </div>
                </td>
                <td class="px-4 py-3.5"><div class="flex items-center justify-center gap-1.5">
                    <button onclick="triggerEdit(this)" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-yellow-100 text-yellow-600 hover:bg-yellow-200 transition-colors"
                        data-action="{{ route('projectm.update', $d->id) }}"
                        data-nama_proyek="{{ $d->nama_proyek }}" data-pic_proyek="{{ $d->pic_proyek }}"
                        data-tujuan="{{ $d->tujuan }}" data-estimasi_waktu="{{ $d->estimasi_waktu }}"
                        data-status="{{ $d->status }}" data-progres="{{ $d->progres }}">
                    ...
                    <button onclick="triggerDelete(this)" type="button" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-red-100 text-red-600 hover:bg-red-200 transition-colors"
                        data-action="{{ route('projectm.destroy', $d->id) }}" data-name="{{ $d->nama_proyek }}">
            ...
            <tr><td colspan="8" class="px-5 py-12 text-center"><div class="flex flex-col items-center gap-3">
                <div class="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center"><i class="fa fa-diagram-project text-2xl text-gray-300"></i></div>
                <p class="text-sm font-medium text-gray-500">Belum ada data proyek</p></div></td></tr>
            @endforelse
        </tbody>
    </table>
...
<div id="mainModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-xl mx-4 max-h-[90vh] overflow-y-auto" style="animation:slideUp .2s ease">
        <div class="flex items-start justify-between px-6 py-5 border-b border-gray-100 sticky top-0 bg-white">
            <h2 id="modalTitle" class="text-base font-bold text-gray-800">Tambah Proyek</h2>
            <button onclick="closeModal()" class="text-gray-400 hover:text-red-500 text-lg leading-none"><i class="fa fa-times"></i></button>
        </div>
        <form id="mainForm" action="{{ route('projectm.store') }}" method="POST" class="px-6 py-5 space-y-4">
            @csrf <div id="methodContainer"></div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Nama Proyek <span class="text-red-500">*</span></label>
                    <input type="text" name="nama_proyek" id="f_nama_proyek" required placeholder="Migrasi ERP" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('nama_proyek') }}"></div>
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">PIC Proyek <span class="text-red-500">*</span></label>
                    <input type="text" name="pic_proyek" id="f_pic_proyek" required placeholder="Nama PIC" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('pic_proyek') }}"></div>
            </div>
            <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Tujuan <span class="text-red-500">*</span></label>
                <textarea name="tujuan" id="f_tujuan" required rows="3" placeholder="Tujuan proyek..." class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 resize-none">{{ old('tujuan') }}</textarea></div>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Estimasi Waktu <span class="text-red-500">*</span></label>
                    <input type="text" name="estimasi_waktu" id="f_estimasi_waktu" required placeholder="3 bulan" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('estimasi_waktu') }}"></div>
                ...
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Progres (%) <span class="text-red-500">*</span></label>
                    <input type="number" name="progres" id="f_progres" required min="0" max="100" placeholder="0" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('progres') }}"></div>
            </div>
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4" style="animation:slideUp .2s ease">
        <div class="px-6 pt-6 pb-2 text-center">
            ...
            <h2 class="text-base font-bold text-gray-800 mt-4">Hapus Proyek?</h2>
...
const createUrl="{{ route('projectm.store') }}";
function openModal(){document.getElementById('modalTitle').innerText='Tambah Proyek';mainForm.action=createUrl;methodContainer.innerHTML='';mainForm.reset();mainModal.classList.remove('hidden');mainModal.classList.add('flex');}
...
function triggerEdit(btn){document.getElementById('modalTitle').innerText='Edit Proyek';mainForm.action=btn.dataset.action;methodContainer.innerHTML='<input type="hidden" name="_method" value="PUT">';
['nama_proyek','pic_proyek','tujuan','estimasi_waktu','status','progres'].forEach(k=>{const el=document.getElementById('f_'+k);if(el)el.value=btn.dataset[k]??'';});mainModal.classList.remove('hidden');mainModal.classList.add('flex');}
