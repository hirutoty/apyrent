        ...
        <form id="mainForm" action="{{ route('emaild.store') }}" method="POST" class="px-6 py-5 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Nama Domain <span class="text-red-500">*</span></label>
                    <input type="text" name="nama_domain" id="f_nama_domain" required placeholder="perusahaan.com" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('nama_domain') }}"></div>
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Provider <span class="text-red-500">*</span></label>
                    <input type="text" name="provider" id="f_provider" required placeholder="GoDaddy / Niagahoster" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('provider') }}"></div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Status <span class="text-red-500">*</span></label>
                    <select name="status" id="f_status" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400">
                        <option value="">- Pilih -</option><option value="aktif" {{ old('status') == 'aktif' ? 'selected' : '' }}>Aktif</option><option value="nonaktif" {{ old('status') == 'nonaktif' ? 'selected' : '' }}>Nonaktif</option></select></div>
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Expired Date</label>
                    <input type="date" name="expired_date" id="f_expired_date" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('expired_date') }}"></div>
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Email Aktif <span class="text-red-500">*</span></label>
                    <input type="number" name="email_aktif" id="f_email_aktif" required min="0" placeholder="50" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('email_aktif') }}"></div>
            </div>
            <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">DNS Terkelola <span class="text-red-500">*</span></label>
                <select name="dns_terkelola" id="f_dns_terkelola" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400">
                    <option value="">- Pilih -</option><option value="1" {{ old('dns_terkelola') == '1' ? 'selected' : '' }}>Ya</option><option value="0" {{ old('dns_terkelola') == '0' ? 'selected' : '' }}>Tidak</option></select></div>
...
function filterTable(){const fS=document.getElementById('filterStatus').value;allRows.forEach(r=>{const match=r.dataset.search.includes(currentSearch)&&(!fS||r.dataset.status===fS);r.style.display=match?'':' none';});}
