...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div><h1 class="text-2xl font-bold text-gray-800">Network Monitoring</h1><p class="text-sm text-gray-500 mt-0.5">Pantau status koneksi jaringan</p></div>
    <button onclick="openModal()" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2.5 rounded-xl shadow-sm transition-colors"><i class="fa fa-plus"></i> Tambah Perangkat</button>
</div>
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        <div class="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center text-blue-500 text-lg flex-shrink-0"><i class="fa fa-network-wired"></i></div>
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalPerangkat }}</p><p class="text-xs text-gray-500 mt-1">Total Perangkat</p></div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        <div class="w-12 h-12 rounded-xl bg-green-50 flex items-center justify-center text-green-500 text-lg flex-shrink-0"><i class="fa fa-wifi"></i></div>
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalOnline }}</p><p class="text-xs text-gray-500 mt-1">Online</p></div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        ...
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalOffline }}</p><p class="text-xs text-gray-500 mt-1">Offline</p></div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        <div class="w-12 h-12 rounded-xl bg-yellow-50 flex items-center justify-center text-yellow-500 text-lg flex-shrink-0"><i class="fa fa-triangle-exclamation"></i></div>
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalWarning }}</p><p class="text-xs text-gray-500 mt-1">Warning</p></div>
    </div>
</div>
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-gray-100">
        <div><h2 class="font-semibold text-gray-800 text-base">Daftar Perangkat Jaringan</h2><p class="text-xs text-gray-400 mt-0.5">{{ $data->count() }} total</p></div>
        <div class="relative"><i class="fa fa-search absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
            <input type="text" placeholder="Cari lokasi/IP..." oninput="onSearch(this.value)" class="pl-8 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 w-44"></div>
    </div>
    <div class="flex flex-wrap items-center gap-3 px-5 py-3 border-b border-gray-100 text-xs">
        <select id="filterStatus" onchange="filterTable()" class="border border-gray-200 rounded-lg px-2 py-1 text-xs focus:outline-none">
            <option value="">Semua Status</option><option>Online</option><option>Offline</option><option>Warning</option><option>Maintenance</option></select>
    </div>
    <div class="overflow-x-auto"><table class="w-full text-sm">
        <thead><tr class="bg-gray-50 border-b border-gray-100">
            ...
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">IP Public</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Status Koneksi</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Bandwidth</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Downtime</th>
        ...
        <tbody id="tableBody">
            ...
            <tr class="border-t border-gray-50 odd:bg-white even:bg-gray-100 hover:bg-blue-50/50 transition-colors"
                data-search="{{ strtolower($d->lokasi.' '.$d->ip_public) }}"
                data-status="{{ $d->status_koneksi }}">
                <td class="px-4 py-3.5 text-gray-400">{{ $data->firstItem() + $loop->index }}</td>
                <td class="px-4 py-3.5 text-sm font-semibold text-gray-800">{{ $d->lokasi }}</td>
                <td class="px-4 py-3.5"><span class="font-mono text-xs bg-gray-100 px-2 py-0.5 rounded">{{ $d->ip_public }}</span></td>
                <td class="px-4 py-3.5">
                    @php $sc=['Online'=>'green','Offline'=>'red','Warning'=>'yellow','Maintenance'=>'blue'][$d->status_koneksi]??'gray' @endphp
                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-{{ $sc }}-100 text-{{ $sc }}-600"><i class="fa fa-circle text-[6px]"></i> {{ $d->status_koneksi }}</span>
                </td>
                <td class="px-4 py-3.5 text-sm text-gray-600">{{ $d->bandwidth }}</td>
                <td class="px-4 py-3.5 text-sm text-gray-600">{{ $d->downtime }}</td>
                ...
                <td class="px-4 py-3.5"><div class="flex items-center justify-center gap-1.5">
                    <button onclick="triggerEdit(this)" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-yellow-100 text-yellow-600 hover:bg-yellow-200 transition-colors"
                        data-action="{{ route('networkm.update', $d->id) }}"
                        data-lokasi="{{ $d->lokasi }}" data-ip_public="{{ $d->ip_public }}"
                        data-status_koneksi="{{ $d->status_koneksi }}" data-bandwidth="{{ $d->bandwidth }}"
                        data-downtime="{{ $d->downtime }}" data-catatan="{{ $d->catatan }}">
                    ...
                    <button onclick="triggerDelete(this)" type="button" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-red-100 text-red-600 hover:bg-red-200 transition-colors"
                        data-action="{{ route('networkm.destroy', $d->id) }}" data-name="{{ $d->lokasi }}">
            ...
            <tr><td colspan="8" class="px-5 py-12 text-center"><div class="flex flex-col items-center gap-3">
                <div class="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center"><i class="fa fa-network-wired text-2xl text-gray-300"></i></div>
                <p class="text-sm font-medium text-gray-500">Belum ada data network monitoring</p></div></td></tr>
            @endforelse
        </tbody>
    </table>
...
<div id="mainModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-xl mx-4 max-h-[90vh] overflow-y-auto" style="animation:slideUp .2s ease">
        <div class="flex items-start justify-between px-6 py-5 border-b border-gray-100 sticky top-0 bg-white">
            <h2 id="modalTitle" class="text-base font-bold text-gray-800">Tambah Perangkat</h2>
            <button onclick="closeModal()" class="text-gray-400 hover:text-red-500 text-lg leading-none"><i class="fa fa-times"></i></button>
        </div>
        <form id="mainForm" action="{{ route('networkm.store') }}" method="POST" class="px-6 py-5 space-y-4">
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4" style="animation:slideUp .2s ease">
        <div class="px-6 pt-6 pb-2 text-center">
            ...
            <h2 class="text-base font-bold text-gray-800 mt-4">Hapus Perangkat?</h2>
...
const createUrl="{{ route('networkm.store') }}";
function openModal(){document.getElementById('modalTitle').innerText='Tambah Perangkat';mainForm.action=createUrl;methodContainer.innerHTML='';mainForm.reset();mainModal.classList.remove('hidden');mainModal.classList.add('flex');}
...
function triggerEdit(btn){document.getElementById('modalTitle').innerText='Edit Perangkat';mainForm.action=btn.dataset.action;methodContainer.innerHTML='<input type="hidden" name="_method" value="PUT">';
['lokasi','ip_public','status_koneksi','bandwidth','downtime','catatan'].forEach(k=>{const el=document.getElementById('f_'+k);if(el)el.value=btn.dataset[k]??'';});mainModal.classList.remove('hidden');mainModal.classList.add('flex');}
