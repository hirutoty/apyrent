...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="flex flex-wrap items-center gap-3 px-5 py-3 border-b border-gray-100 text-xs">
        ...
        <select id="filterLevel" onchange="filterTable()" class="border border-gray-200 rounded-lg px-2 py-1 text-xs focus:outline-none">
        ...
        <form id="mainForm" action="{{ route('cybers.store') }}" method="POST" class="px-6 py-5 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Area Diaudit <span class="text-red-500">*</span></label>
                    <input type="text" name="area_diaudit" id="f_area_diaudit" required placeholder="Jaringan / Aplikasi / Data Center" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('area_diaudit') }}"></div>
            </div>
            <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Temuan Risiko <span class="text-red-500">*</span></label>
                <textarea name="temuan_risiko" id="f_temuan_risiko" required rows="3" placeholder="Deskripsi temuan risiko yang ditemukan..." class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 resize-none">{{ old('temuan_risiko') }}</textarea></div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Tindakan Perbaikan <span class="text-red-500">*</span></label>
                <textarea name="tindakan_perbaikan" id="f_tindakan_perbaikan" required rows="3" placeholder="Langkah perbaikan yang dilakukan..." class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 resize-none">{{ old('tindakan_perbaikan') }}</textarea></div>
...
function filterTable(){const fS=document.getElementById('filterStatus').value;const fL=document.getElementById('filterLevel').value;allRows.forEach(r=>{const matchSearch=r.dataset.search.includes(currentSearch);const matchStatus=!fS||r.dataset.status===fS;const matchLevel=!fL||r.dataset.levelRisiko===fL;r.style.display=(matchSearch&&matchStatus&&matchLevel)?'':'none';});}
