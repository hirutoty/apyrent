...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div><h1 class="text-2xl font-bold text-gray-800">Software License</h1><p class="text-sm text-gray-500 mt-0.5">Kelola lisensi perangkat lunak perusahaan</p></div>
    <button onclick="openModal()" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2.5 rounded-xl shadow-sm transition-colors"><i class="fa fa-plus"></i> Tambah Lisensi</button>
</div>
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        <div class="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center text-blue-500 text-lg flex-shrink-0"><i class="fa fa-file-code"></i></div>
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalLisensi }}</p><p class="text-xs text-gray-500 mt-1">Total Lisensi</p></div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        ...
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalExpired }}</p><p class="text-xs text-gray-500 mt-1">Expired</p></div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        <div class="w-12 h-12 rounded-xl bg-purple-50 flex items-center justify-center text-purple-500 text-lg flex-shrink-0"><i class="fa fa-layer-group"></i></div>
        <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalUnit }}</p><p class="text-xs text-gray-500 mt-1">Total Unit</p></div>
    </div>
</div>
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-gray-100">
        <div><h2 class="font-semibold text-gray-800 text-base">Daftar Software License</h2><p class="text-xs text-gray-400 mt-0.5">{{ $data->count() }} total</p></div>
        <div class="relative"><i class="fa fa-search absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
            <input type="text" placeholder="Cari software..." oninput="onSearch(this.value)" class="pl-8 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 w-44"></div>
    </div>
    <div class="flex flex-wrap items-center gap-3 px-5 py-3 border-b border-gray-100 text-xs">        <select id="filterStatus" onchange="filterTable()" class="border border-gray-200 rounded-lg px-2 py-1 text-xs focus:outline-none">
            <option value="">Semua Status</option><option>Aktif</option><option>Expired</option></select>
    </div>
    <div class="overflow-x-auto"><table class="w-full text-sm">
        <thead><tr class="bg-gray-50 border-b border-gray-100">
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">No</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Nama Software</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Jenis Lisensi</th>
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Jumlah</th>
            ...
            <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Tgl Perpanjangan</th>
            <th class="text-center text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Aksi</th>
        </tr></thead>
        <tbody id="tableBody">
            @forelse($data as $d)
            <tr class="border-t border-gray-50 odd:bg-white even:bg-gray-100 hover:bg-blue-50/50 transition-colors"
                data-search="{{ strtolower($d->nama_software.' '.$d->jenis_lisensi.' '.$d->provider) }}"
                data-status="{{ $d->status }}">
                <td class="px-4 py-3.5 text-gray-400">{{ $data->firstItem() + $loop->index }}</td>
                <td class="px-4 py-3.5 text-sm font-semibold text-gray-800">{{ $d->nama_software }}</td>
                <td class="px-4 py-3.5 text-sm text-gray-600">{{ $d->jenis_lisensi }}</td>
                <td class="px-4 py-3.5 text-sm text-gray-700">{{ $d->jumlah_lisensi }}</td>
                <td class="px-4 py-3.5 text-sm text-gray-600">{{ $d->provider }}</td>
                <td class="px-4 py-3.5 text-sm text-gray-500">{{ $d->masa_berlaku ? \Carbon\Carbon::parse($d->masa_berlaku)->format('d/m/Y') : '-' }}</td>
                <td class="px-4 py-3.5">
                    @php $sc=['Aktif'=>'green','Expired'=>'red'][$d->status]??'gray' @endphp
                ...
                <td class="px-4 py-3.5 text-sm text-gray-500">{{ $d->tanggal_perpanjangan ? \Carbon\Carbon::parse($d->tanggal_perpanjangan)->format('d/m/Y') : '-' }}</td>
                <td class="px-4 py-3.5"><div class="flex items-center justify-center gap-1.5">
                    <button onclick="triggerEdit(this)" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-yellow-100 text-yellow-600 hover:bg-yellow-200 transition-colors"
                        data-action="{{ route('softwarel.update', $d->id) }}"
                        data-nama_software="{{ $d->nama_software }}" data-jenis_lisensi="{{ $d->jenis_lisensi }}"
                        data-jumlah_lisensi="{{ $d->jumlah_lisensi }}" data-provider="{{ $d->provider }}"
                        data-masa_berlaku="{{ $d->masa_berlaku }}" data-status="{{ $d->status }}"
                        data-tanggal_perpanjangan="{{ $d->tanggal_perpanjangan }}">
                    ...
                    <button onclick="triggerDelete(this)" type="button" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-red-100 text-red-600 hover:bg-red-200 transition-colors"
                        data-action="{{ route('softwarel.destroy', $d->id) }}" data-name="{{ $d->nama_software }}">
                        <i class="fa fa-trash text-xs"></i> Hapus</button>
                </div></td>
            </tr>
            @empty
            <tr><td colspan="9" class="px-5 py-12 text-center"><div class="flex flex-col items-center gap-3">
                <div class="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center"><i class="fa fa-file-code text-2xl text-gray-300"></i></div>
                <p class="text-sm font-medium text-gray-500">Belum ada data software license</p></div></td></tr>
            @endforelse
        </tbody>
    </table>
...
<div id="mainModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-xl mx-4 max-h-[90vh] overflow-y-auto" style="animation:slideUp .2s ease">
        <div class="flex items-start justify-between px-6 py-5 border-b border-gray-100 sticky top-0 bg-white">
            <h2 id="modalTitle" class="text-base font-bold text-gray-800">Tambah Software License</h2>
            <button onclick="closeModal()" class="text-gray-400 hover:text-red-500 text-lg leading-none"><i class="fa fa-times"></i></button>
        </div>
        <form id="mainForm" action="{{ route('softwarel.store') }}" method="POST" class="px-6 py-5 space-y-4">
            @csrf <div id="methodContainer"></div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Nama Software <span class="text-red-500">*</span></label>
                    <input type="text" name="nama_software" id="f_nama_software" required placeholder="Microsoft Office 365" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('nama_software') }}"></div>
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Jenis Lisensi <span class="text-red-500">*</span></label>
                    <input type="text" name="jenis_lisensi" id="f_jenis_lisensi" required placeholder="Subscription / Perpetual" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('jenis_lisensi') }}"></div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Jumlah Lisensi <span class="text-red-500">*</span></label>
                    <input type="number" name="jumlah_lisensi" id="f_jumlah_lisensi" required min="1" placeholder="10" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('jumlah_lisensi') }}"></div>
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Provider <span class="text-red-500">*</span></label>
                    <input type="text" name="provider" id="f_provider" required placeholder="Microsoft / Adobe" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('provider') }}"></div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Masa Berlaku <span class="text-red-500">*</span></label>
                    <input type="date" name="masa_berlaku" id="f_masa_berlaku" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('masa_berlaku') }}"></div>
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Status <span class="text-red-500">*</span></label>
                    <select name="status" id="f_status" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400">
                        <option value="">- Pilih -</option><option>Aktif</option><option>Expired</option></select></div>
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Tgl Perpanjangan</label>
                    <input type="date" name="tanggal_perpanjangan" id="f_tanggal_perpanjangan" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('tanggal_perpanjangan') }}"></div>
            </div>
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4" style="animation:slideUp .2s ease">
        <div class="px-6 pt-6 pb-2 text-center">
            ...
            <h2 class="text-base font-bold text-gray-800 mt-4">Hapus Lisensi?</h2>
...
const createUrl="{{ route('softwarel.store') }}";
function openModal(){document.getElementById('modalTitle').innerText='Tambah Software License';mainForm.action=createUrl;methodContainer.innerHTML='';mainForm.reset();mainModal.classList.remove('hidden');mainModal.classList.add('flex');}
