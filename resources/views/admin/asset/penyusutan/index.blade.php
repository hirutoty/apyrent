...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Penyusutan Asset</h1>
            <p class="text-sm text-gray-500 mt-0.5">Kelola data penyusutan nilai aset per tahun</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center text-blue-500 text-lg flex-shrink-0"><i class="bi bi-graph-down-arrow"></i></div>
            <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $total }}</p><p class="text-xs text-gray-500 mt-1">Total Record</p></div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl bg-green-50 flex items-center justify-center text-green-500 text-lg flex-shrink-0"><i class="bi bi-cash"></i></div>
            <div><p class="text-sm font-bold text-gray-800 leading-none">Rp {{ number_format($totalNilaiAwal,0,',','.') }}</p><p class="text-xs text-gray-500 mt-1">Total Nilai Awal</p></div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl bg-orange-50 flex items-center justify-center text-orange-500 text-lg flex-shrink-0"><i class="bi bi-arrow-down-circle-fill"></i></div>
            <div><p class="text-sm font-bold text-gray-800 leading-none">Rp {{ number_format($totalAkumulasi,0,',','.') }}</p><p class="text-xs text-gray-500 mt-1">Total Akumulasi</p></div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl bg-purple-50 flex items-center justify-center text-purple-500 text-lg flex-shrink-0"><i class="bi bi-wallet2"></i></div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-gray-100">
            <div><h2 class="font-semibold text-gray-800 text-base">Daftar Penyusutan</h2><p class="text-xs text-gray-400 mt-0.5">{{ $data->count() }} total record</p></div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100">
                        ...
                        <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Tahun</th>
                        <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Nilai Awal</th>
                        <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Akumulasi Penyusutan</th>
                ...
                <tbody id="tableBody">
                    ...
                    <tr class="border-t border-gray-50 odd:bg-white even:bg-gray-100 hover:bg-blue-50/50 transition-colors"
                        data-search="{{ strtolower($d->kode_aset.' '.$d->tahun.' '.$d->metode.' '.$d->status) }}">
                        ...
                        <td class="px-4 py-3.5 font-bold text-gray-700">{{ $d->tahun }}</td>
                        <td class="px-4 py-3.5 text-sm text-gray-700">Rp {{ number_format($d->nilai_awal,0,',','.') }}</td>
                        <td class="px-4 py-3.5 text-sm text-orange-600 font-medium">Rp {{ number_format($d->akumulasi_penyusutan,0,',','.') }}</td>
                        <td class="px-4 py-3.5 text-sm text-purple-600 font-medium">Rp {{ number_format($d->nilai_buku,0,',','.') }}</td>
                        <td class="px-4 py-3.5 text-sm text-gray-600">{{ $d->metode }}</td>
                        ...
                        <td class="px-4 py-3.5">
                            <div class="flex items-center justify-center gap-1.5">
                                <button class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-yellow-100 text-yellow-600 hover:bg-yellow-200 transition-colors"
                                    data-action="{{ route('asset.penyusutan.update', $d->id) }}"
                                    data-kode_aset="{{ $d->kode_aset }}" data-tahun="{{ $d->tahun }}"
                                    data-nilai_awal="{{ $d->nilai_awal }}" data-akumulasi_penyusutan="{{ $d->akumulasi_penyusutan }}"
                                    data-nilai_buku="{{ $d->nilai_buku }}" data-metode="{{ $d->metode }}" data-status="{{ $d->status }}"
                                ...
                                <button type="button" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-red-100 text-red-600 hover:bg-red-200 transition-colors"
                                    data-action="{{ route('asset.penyusutan.destroy', $d->id) }}" data-name="{{ $d->kode_aset.' ('.$d->tahun.')' }}"
                    ...
                    <tr><td colspan="9" class="px-5 py-12 text-center">
                        <div class="flex flex-col items-center gap-3"><div class="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center"><i class="bi bi-graph-down-arrow text-2xl text-gray-300"></i></div>
                        <p class="text-sm font-medium text-gray-500">Belum ada data penyusutan</p></div>
                    </td></tr>
                    @endforelse
                </tbody>
            </table>
...
<div id="mainModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto" style="animation:slideUp .2s ease">
        <div class="flex items-start justify-between px-6 py-5 border-b border-gray-100 sticky top-0 bg-white">
            <h2 id="modalTitle" class="text-base font-bold text-gray-800">Tambah Penyusutan</h2>
            <button onclick="closeModal()" class="text-gray-400 hover:text-red-500 text-lg"><i class="bi bi-x-lg"></i></button>
        </div>
        <form id="mainForm" action="{{ route('asset.penyusutan.store') }}" method="POST" class="px-6 py-5 space-y-4">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Nilai Awal (Rp) <span class="text-red-500">*</span></label>
                    <input type="number" max="9999999999" min="0" step="0.01" name="nilai_awal" id="f_nilai_awal" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('nilai_awal') }}"></div>
                ...
                <div><label class="block text-xs font-semibold text-gray-600 mb-1.5">Nilai Buku (Rp) <span class="text-red-500">*</span></label>
                    <input type="number" max="9999999999" min="0" step="0.01" name="nilai_buku" id="f_nilai_buku" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('nilai_buku') }}"></div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4" style="animation:slideUp .2s ease">
        <div class="px-6 pt-6 pb-2 text-center">
            ...
            <p class="text-xs text-gray-500 mt-1.5">Kamu akan menghapus penyusutan <strong id="deleteName" class="text-gray-700"></strong>.</p>
        </div>
...
<script>
const mainModal=document.getElementById('mainModal'),mainForm=document.getElementById('mainForm'),methodContainer=document.getElementById('methodContainer'),createUrl="{{ route('asset.penyusutan.store') }}";
function openModal(){document.getElementById('modalTitle').innerText='Tambah Penyusutan';mainForm.action=createUrl;methodContainer.innerHTML='';mainForm.reset();mainModal.classList.remove('hidden');mainModal.classList.add('flex');}
...
function triggerEdit(b){document.getElementById('modalTitle').innerText='Edit Penyusutan';mainForm.action=b.dataset.action;methodContainer.innerHTML='<input type="hidden" name="_method" value="PUT">';
['kode_aset','tahun','nilai_awal','akumulasi_penyusutan','nilai_buku','metode','status'].forEach(k=>{let el=document.getElementById('f_'+k);if(el)el.value=b.dataset[k]??'';});
