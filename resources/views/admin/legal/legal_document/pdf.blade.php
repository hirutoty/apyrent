...
<body>
    ...
    <table>
        <thead>
            <tr>
                <th>No</th><th>Kode</th><th>Nama Dokumen</th><th>Jenis</th><th>Pihak Terkait</th><th>Tgl Terbit</th><th>Berlaku Hingga</th><th>Format</th><th>Status</th>
            </tr>
        </thead>
        <tbody>
            @forelse($data as $i => $d)
            <tr>
                ...
                <td>{{ \Carbon\Carbon::parse($d->tgl_terbit)->format('d/m/Y') }}</td>
                <td>{{ $d->berlaku_hingga ? \Carbon\Carbon::parse($d->berlaku_hingga)->format('d/m/Y') : '-' }}</td>
                <td>{{ $d->format }}</td>
                <td><span class="badge {{ $d->status==='Aktif' ? 'badge-green' : ($d->status==='Kadaluarsa' ? 'badge-red' : 'badge-gray') }}">{{ $d->status }}</span></td>
            </tr>
            @empty
    ...
    <div class="footer">{{ $setting?->nama_perusahaan ?? 'APY Rent' }} &mdash; Laporan Dokumen Legal</div>
</body>
</html>
