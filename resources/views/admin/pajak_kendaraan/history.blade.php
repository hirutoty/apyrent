...
@section('title', 'History Pajak Kendaraan')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">History Perpanjangan Pajak</h1>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
            ...
            <h3 class="text-sm font-bold text-blue-600 mt-1">
                Rp {{ number_format(\App\Models\PajakHistory::when($bulan !== 'semua', fn($q) => $q->whereMonth('jatuh_tempo', $bulan))->when($tahun !== 'semua', fn($q) => $q->whereYear('jatuh_tempo', $tahun))->sum('nominal'), 0, ',', '.') }}
            </h3>
    ...
    {{-- CHART FILTER --}}
    <x-chart-filter id="pajakHistoryChartFilter" defaultFilter="month" :showCustomRange="true" />

    {{-- CHART CONTAINER --}}
    <x-chart-container
        id="pajakHistoryChartContainer"
        layout="stacked"
        pieTitle="Distribusi Jenis Pajak"   pieId="pajakHistoryPieChart"
        barTitle="Biaya Pajak per Periode"  barId="pajakHistoryBarChart"
        lineTitle="Trend Biaya Pajak"       lineId="pajakHistoryLineChart"
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-400">Jenis Pajak</th>
                        <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-gray-400">Biaya</th>
                        <th class="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wide text-gray-400">Jatuh Tempo</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($data as $item)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 text-gray-700">{{ $item->jenis_pajak }}</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    @if($item->kendaraan_id)
                                        <a href="{{ route('kendaraan.pajak-history', $item->kendaraan_id) }}"
                        ...
                        <tr>
                            <td colspan="10" class="py-16 text-center">
                                <div class="flex flex-col items-center gap-2 text-gray-400">
                                    ...
                                    <p class="text-sm font-medium">Belum ada history perpanjangan.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
document.addEventListener('DOMContentLoaded', function () {
    initPajakHistoryCharts({ filter_type: 'month' });

    document.addEventListener('chartFilterChange', function (e) {
        if (e.detail.filterId === 'pajakHistoryChartFilter') {
            updatePajakHistoryCharts({
...
async function initPajakHistoryCharts(filters) {
    try {
        await chartManager.initChartsFromAPI('pajak-history', {
            pie:  'pajakHistoryPieChart',
            bar:  'pajakHistoryBarChart',
            line: 'pajakHistoryLineChart',
        }, filters, { accentLine: true });
    } catch (e) { console.error('Error loading pajak history charts:', e); }
}

async function updatePajakHistoryCharts(filters) {
    try {
        const scrollable = filters.filter_type === 'custom';
        await chartManager.updateChartsFromAPI('pajak-history', {
            pie:  'pajakHistoryPieChart',
            bar:  'pajakHistoryBarChart',
            line: 'pajakHistoryLineChart',
        }, filters, { scrollable, accentLine: true }, { scrollable });
    } catch (e) { console.error('Error updating pajak history charts:', e); }
}
</script>
