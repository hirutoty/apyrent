...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Kelola data jabatan dan struktur pegawai</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl bg-green-50 flex items-center justify-center text-green-500 text-lg flex-shrink-0"><i class="fa fa-id-badge"></i></div>
            <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalTetap }}</p><p class="text-xs text-gray-500 mt-1">Pegawai Tetap</p></div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl bg-yellow-50 flex items-center justify-center text-yellow-500 text-lg flex-shrink-0"><i class="fa fa-file-contract"></i></div>
            <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalKontrak }}</p><p class="text-xs text-gray-500 mt-1">Pegawai Kontrak</p></div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
            ...
            <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $totalDepartemen }}</p><p class="text-xs text-gray-500 mt-1">Departemen</p></div>
        </div>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-gray-100">
            <div><h2 class="font-semibold text-gray-800 text-base">Daftar Struktur Organisasi</h2><p class="text-xs text-gray-400 mt-0.5">{{ $data->count() }} total data</p></div>
            <div class="relative">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100">
                        ...
                        <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">NIP / ID</th>
                        <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Jabatan</th>
                        ...
                        <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Mulai</th>
                ...
                <tbody id="tableBody">
                    @forelse($data as $d)
                        <tr class="border-t border-gray-50 odd:bg-white even:bg-gray-100 hover:bg-blue-50/50 transition-colors"
                            data-search="{{ strtolower($d->nama_pegawai . ' ' . $d->departemen . ' ' . $d->nama_jabatan) }}">
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex items-center gap-2">
                                    <div class="w-7 h-7 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
                                        <i class="fa fa-user text-blue-400 text-xs"></i>
                                    </div>
                                    <span class="font-semibold text-gray-800">{{ $d->nama_pegawai }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3.5"><span class="font-mono text-xs text-gray-600 bg-gray-100 px-2 py-0.5 rounded">{{ $d->nip_id }}</span></td>
                            <td class="px-4 py-3.5 text-gray-700">{{ $d->nama_jabatan }}</td>
                            <td class="px-4 py-3.5 text-gray-600">{{ $d->departemen }}</td>
                            <td class="px-4 py-3.5">
                                @if($d->status_jabatan === 'Tetap')
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-green-100 text-green-600"><i class="fa fa-circle text-[6px]"></i> Tetap</span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-600"><i class="fa fa-circle text-[6px]"></i> Kontrak</span>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex items-center justify-center gap-1.5">
                                    <button class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-yellow-100 text-yellow-600 hover:bg-yellow-200 transition-colors"
                                        data-action="{{ route('struktur.update', $d->id) }}"
                                        data-nama_jabatan="{{ $d->nama_jabatan }}"
                                        data-nama_pegawai="{{ $d->nama_pegawai }}"
                                        data-nip_id="{{ $d->nip_id }}"
                                        data-departemen="{{ $d->departemen }}"
                                        data-atasan_langsung="{{ $d->atasan_langsung }}"
                                        data-lokasi="{{ $d->lokasi }}"
                                        data-status_jabatan="{{ $d->status_jabatan }}"
                                    ...
                                    <button type="button" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-red-100 text-red-600 hover:bg-red-200 transition-colors"
                                        data-action="{{ route('struktur.destroy', $d->id) }}"
                                        data-name="{{ $d->nama_pegawai }}"
                    ...
                    @empty
                        <tr><td colspan="8" class="px-5 py-12 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p class="text-sm font-medium text-gray-500">Belum ada data struktur organisasi</p>
                            </div>
                        </td></tr>
                    @endforelse
                </tbody>
...
<div id="mainModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" style="animation:slideUp .2s ease">
        <div class="flex items-start justify-between px-6 py-5 border-b border-gray-100 sticky top-0 bg-white">
            <h2 id="modalTitle" class="text-base font-bold text-gray-800">Tambah Data</h2>
            <button onclick="closeModal()" class="text-gray-400 hover:text-red-500 transition-colors text-lg"><i class="fa fa-times"></i></button>
        </div>
        <form id="mainForm" action="{{ route('struktur.store') }}" method="POST" class="px-6 py-5 space-y-4">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">NIP / ID <span class="text-red-500">*</span></label>
                    <input type="text" name="nip_id" id="f_nip_id" required placeholder="Contoh: 2024001"
                        class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('nip_id') }}"
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Jabatan <span class="text-red-500">*</span></label>
                    <input type="text" name="nama_jabatan" id="f_nama_jabatan" required placeholder="Contoh: Manager HR"
                        class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('nama_jabatan') }}"
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Departemen <span class="text-red-500">*</span></label>
                    <input type="text" name="departemen" id="f_departemen" required placeholder="Contoh: HR"
                        class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('departemen') }}"
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Atasan Langsung</label>
                    <input type="text" name="atasan_langsung" id="f_atasan_langsung" placeholder="Nama atasan (opsional)"
                        class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400" value="{{ old('atasan_langsung') }}"
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Lokasi <span class="text-red-500">*</span></label>
                    <input type="text" name="lokasi" id="f_lokasi" required placeholder="Contoh: Kantor Pusat"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Status Jabatan <span class="text-red-500">*</span></label>
                    <select name="status_jabatan" id="f_status_jabatan" required
                        class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400">
                        <option value="">- Pilih -</option>
                        <option value="Tetap" {{ old('status_jabatan') == 'Tetap' ? 'selected' : '' }}>Tetap</option>
                        <option value="Kontrak" {{ old('status_jabatan') == 'Kontrak' ? 'selected' : '' }}>Kontrak</option>
                    </select>
                </div>
                <div>
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4" style="animation:slideUp .2s ease">
        <div class="px-6 pt-6 pb-2 text-center">
            ...
            <p class="text-xs text-gray-500 mt-1.5">Menghapus data <strong id="deleteName"></strong>. Tidak dapat dibatalkan.</p>
        </div>
...
function openModal(){document.getElementById('modalTitle').innerText='Tambah Data';mainForm.action="{{route('struktur.store')}}";methodContainer.innerHTML='';mainForm.reset();mainModal.classList.remove('hidden');mainModal.classList.add('flex');}
...
function triggerEdit(btn){
    ...
    ['nama_pegawai','nip_id','nama_jabatan','departemen','atasan_langsung','lokasi','status_jabatan','tanggal_mulai']
