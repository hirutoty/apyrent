...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">History Perpanjangan KIR</h1>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
            ...
            <h3 class="text-sm font-bold text-amber-600 mt-1">
                Rp {{ number_format(\App\Models\KirHistory::when($bulan !== 'semua', fn($q) => $q->whereMonth('masa_berlaku', $bulan))->when($tahun !== 'semua', fn($q) => $q->whereYear('masa_berlaku', $tahun))->sum('biaya'), 0, ',', '.') }}
            </h3>
    ...
    {{-- CHART FILTER --}}
    <x-chart-filter id="kirHistoryChartFilter" defaultFilter="month" :showCustomRange="true" />

    {{-- CHART CONTAINER --}}
    <x-chart-container
        id="kirHistoryChartContainer"
        layout="stacked"
        pieTitle="Distribusi Nomor Uji"   pieId="kirHistoryPieChart"
        barTitle="Biaya KIR per Periode"  barId="kirHistoryBarChart"
        lineTitle="Trend Biaya KIR"       lineId="kirHistoryLineChart"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 px-5 py-4">
        <form method="GET" action="{{ route('history.kir.index') }}" class="flex flex-wrap items-end gap-3">
            ...
            <select name="tahun" onchange="this.form.submit()"
                class="rounded-lg border border-gray-300 text-sm px-3 py-2 focus:ring-blue-500 focus:border-blue-500">
                <option value="semua" {{ $bulan == 'semua' ? 'selected' : '' }}>Semua Tahun</option>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-400">No Uji</th>
                        <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-gray-400">Biaya</th>
                        <th class="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wide text-gray-400">Masa Berlaku Lama</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($data as $item)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 text-center text-gray-600 whitespace-nowrap">
                                {{ $item->kir?->tanggal_bayar ? \Carbon\Carbon::parse($item->kir->tanggal_bayar)->format('d M Y') : '-' }}
                            </td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    @if($item->kendaraan_id)
                                        <a href="{{ route('kendaraan.kir-history', $item->kendaraan_id) }}"
                        ...
                        <tr>
                            <td colspan="10" class="py-16 text-center">
                                <div class="flex flex-col items-center gap-2 text-gray-400">
                                    ...
                                    <p class="text-sm font-medium">Belum ada history perpanjangan KIR.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
document.addEventListener('DOMContentLoaded', function () {
    initKirHistoryCharts({ filter_type: 'month' });

    document.addEventListener('chartFilterChange', function (e) {
        if (e.detail.filterId === 'kirHistoryChartFilter') {
            updateKirHistoryCharts({
...
async function initKirHistoryCharts(filters) {
    try {
        await chartManager.initChartsFromAPI('kir-history', {
            pie:  'kirHistoryPieChart',
            bar:  'kirHistoryBarChart',
            line: 'kirHistoryLineChart',
        }, filters, { accentLine: true });
    } catch (e) { console.error('Error loading KIR history charts:', e); }
}

async function updateKirHistoryCharts(filters) {
    try {
        const scrollable = filters.filter_type === 'custom';
        await chartManager.updateChartsFromAPI('kir-history', {
            pie:  'kirHistoryPieChart',
            bar:  'kirHistoryBarChart',
            line: 'kirHistoryLineChart',
        }, filters, { scrollable, accentLine: true }, { scrollable });
    } catch (e) { console.error('Error updating KIR history charts:', e); }
}
</script>
