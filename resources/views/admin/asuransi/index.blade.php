...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Data Asuransi</h1>
            <p class="text-sm text-gray-500 mt-0.5">Kelola data perusahaan asuransi kendaraan</p>
        </div>
                        ...
                        <tr class="border-t border-gray-50 hover:bg-gray-50 transition-colors duration-100"
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex items-center gap-2">
                                    <div class="w-7 h-7 rounded-lg bg-blue-50 flex items-center justify-center flex-shrink-0">
                                        <i class="fa fa-shield-alt text-blue-400 text-xs"></i>
                                    </div>
                                    <span class="text-sm font-semibold text-gray-800">{{ $d->nama_asuransi }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex items-center justify-center gap-1.5">
                                    ...
                                    <form action="/admin/asuransi/{{ $d->id }}" method="POST"
                        ...
                        <tr>
                            <td colspan="8" class="px-5 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-xs text-gray-400">Klik "Tambah Asuransi" untuk menambahkan data baru</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
{{-- ======================================
    MODAL TAMBAH / EDIT ASURANSI
======================================--}}
<div id="asuransiModal"
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4"
        ...
        <div class="flex items-start justify-between px-6 py-5 border-b border-gray-100">
            <div>
                <h2 id="modalTitle" class="text-base font-bold text-gray-800">Tambah Asuransi</h2>
                <p class="text-xs text-gray-500 mt-0.5">Isi data perusahaan asuransi kendaraan</p>
            </div>
        ...
        <form id="asuransiForm" action="/admin/asuransi" method="POST" class="px-6 py-5 space-y-4">
            @csrf
...
function openModal() {
    document.getElementById('modalTitle').innerText = 'Tambah Asuransi';
    asuransiForm.action = '/admin/asuransi';
    methodContainer.innerHTML = '';
    asuransiForm.reset();
    asuransiModal.classList.remove('hidden');
    asuransiModal.classList.add('flex');
}

function closeModal() {
    asuransiModal.classList.add('hidden');
    asuransiModal.classList.remove('flex');
}

asuransiModal.addEventListener('click', function (e) {
    if (e.target === asuransiModal) closeModal();
});
...
document.querySelectorAll('.btn-edit').forEach(btn => {
    btn.addEventListener('click', function () {
        document.getElementById('modalTitle').innerText = 'Edit Asuransi';
        asuransiForm.action = '/admin/asuransi/' + this.dataset.id;
        methodContainer.innerHTML = '<input type="hidden" name="_method" value="PUT">';

        document.getElementById('f_nama_asuransi').value    = this.dataset.nama_asuransi;
        document.getElementById('f_alamat').value            = this.dataset.alamat;
        document.getElementById('f_nama_marketing').value   = this.dataset.nama_marketing;
        document.getElementById('f_kontak_marketing').value = this.dataset.kontak_marketing;
        document.getElementById('f_nama_bengkel').value     = this.dataset.nama_bengkel;
        document.getElementById('f_kontak_bengkel').value   = this.dataset.kontak_bengkel;
