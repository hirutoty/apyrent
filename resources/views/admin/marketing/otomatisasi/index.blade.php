...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Otomatisasi Marketing</h1>
        ...
        <div class="flex items-center gap-2">
            ...
            <button onclick="openModal()" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2.5 rounded-xl shadow-sm transition-colors">
                <i class="fa fa-plus text-sm"></i> Tambah Workflow
            </button>
        </div>
    </div>
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center text-blue-500 text-lg flex-shrink-0"><i class="fa fa-robot"></i></div>
            <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $total }}</p><p class="text-xs text-gray-500 mt-1">Total Workflow</p></div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-gray-100">
            <div><h2 class="font-semibold text-gray-800 text-base">Daftar Workflow</h2><p class="text-xs text-gray-400 mt-0.5">{{ $data->count() }} total data</p></div>
            <div class="relative"><i class="fa fa-search absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
            <input type="text" placeholder="Cari workflow..." oninput="onSearchInput(this.value)" class="pl-8 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 w-44"></div>
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100">
                        ...
                        <th class="text-left text-xs font-semibold uppercase text-gray-500 px-4 py-3">Workflow ID</th>
                        <th class="text-left text-xs font-semibold uppercase text-gray-500 px-4 py-3">Nama</th>
                        <th class="text-left text-xs font-semibold uppercase text-gray-500 px-4 py-3">Trigger Event</th>
                        <th class="text-left text-xs font-semibold uppercase text-gray-500 px-4 py-3">Syarat Tambahan</th>
                        <th class="text-left text-xs font-semibold uppercase text-gray-500 px-4 py-3">Aksi</th>
                        <th class="text-left text-xs font-semibold uppercase text-gray-500 px-4 py-3">Delay Aksi</th>
                ...
                <tbody id="tableBody">
                    @forelse($data as $d)
                    <tr class="border-t border-gray-50 hover:bg-gray-50 transition-colors" data-search="{{ strtolower($d->nama_workflow.' '.$d->workflow_id.' '.$d->trigger_event.' '.$d->status.' '.$d->pic) }}">
                        ...
                        <td class="px-4 py-3.5 text-xs text-gray-500">{{ $d->syarat_tambahan ?? '-' }}</td>
                        <td class="px-4 py-3.5 text-xs text-gray-600">{{ $d->aksi }}</td>
                        <td class="px-4 py-3.5 text-xs text-gray-500">{{ $d->delay_aksi ?? '-' }}</td>
                        ...
                        <td class="px-4 py-3.5 text-xs text-gray-500 max-w-[160px] truncate" title="{{ $d->catatan }}">{{ $d->catatan ?? '-' }}</td>
                        <td class="px-4 py-3.5">
                            <div class="flex items-center justify-center gap-1.5">
                                <button class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-yellow-100 text-yellow-600 hover:bg-yellow-200 transition-colors"
                                    data-action="{{ route('otomatisasi.update', $d->id) }}" data-workflow_id="{{ $d->workflow_id }}"
                                    data-nama_workflow="{{ $d->nama_workflow }}" data-trigger_event="{{ $d->trigger_event }}"
                                    data-syarat_tambahan="{{ $d->syarat_tambahan }}" data-aksi="{{ $d->aksi }}"
                                    data-delay_aksi="{{ $d->delay_aksi }}" data-status="{{ $d->status }}"
                                    data-pic="{{ $d->pic }}" data-catatan="{{ $d->catatan }}" onclick="triggerEdit(this)">
                                ...
                                <button type="button" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-red-100 text-red-600 hover:bg-red-200 transition-colors"
                                    data-action="{{ route('otomatisasi.destroy', $d->id) }}" data-name="{{ $d->nama_workflow }}" onclick="triggerDelete(this)">
                    ...
                    @empty
                    <tr><td colspan="11" class="px-5 py-12 text-center"><div class="flex flex-col items-center gap-3"><div class="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center"><i class="fa fa-robot text-2xl text-gray-300"></i></div><p class="text-sm font-medium text-gray-500">Belum ada data workflow</p></div></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
...
<div id="mainModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-xl mx-4 max-h-[90vh] overflow-y-auto">
        <div class="flex items-start justify-between px-6 py-5 border-b border-gray-100 sticky top-0 bg-white">
            <div><h2 id="modalTitle" class="text-base font-bold text-gray-800">Tambah Workflow</h2><p class="text-xs text-gray-500 mt-0.5">Isi data workflow otomatisasi</p></div>
        ...
        <form id="mainForm" action="{{ route('otomatisasi.store') }}" method="POST" class="px-6 py-5 space-y-4">
            @csrf
...
<div id="alertOverlay" class="fixed inset-0 z-[9999] flex items-start justify-center pt-6" style="background:rgba(0,0,0,0.18);opacity:0;transition:opacity 0.2s;pointer-events:none">
    <div id="alertBox" class="bg-white rounded-xl shadow-xl border border-gray-100 px-5 py-4 flex items-start gap-3 w-full max-w-md mx-4" style="transform:translateY(-16px);transition:transform 0.25s">
        ...
        <div class="flex-1"><p class="text-sm font-bold text-gray-800">Terjadi Kesalahan!</p><ul class="text-xs text-gray-500 mt-0.5 list-disc ml-4">@foreach($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul></div>
        @endif
...
@endif
<script>
const mainModal=document.getElementById('mainModal'),mainForm=document.getElementById('mainForm'),methodContainer=document.getElementById('methodContainer'),createUrl="{{ route('otomatisasi.store') }}";
function openModal(){document.getElementById('modalTitle').innerText='Tambah Workflow';mainForm.action=createUrl;methodContainer.innerHTML='';mainForm.reset();mainModal.classList.remove('hidden');mainModal.classList.add('flex');}
...
function triggerEdit(btn){document.getElementById('modalTitle').innerText='Edit Workflow';mainForm.action=btn.dataset.action;methodContainer.innerHTML='<input type="hidden" name="_method" value="PUT">';
['workflow_id','nama_workflow','trigger_event','syarat_tambahan','aksi','delay_aksi','status','pic','catatan'].forEach(k=>{const el=document.getElementById('f_'+k);if(el)el.value=btn.dataset[k]??'';});
