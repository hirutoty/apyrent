...
<head>
    <meta charset="UTF-8">
    <title>Data Otomatisasi Marketing</title>
    <style>
...
<body>
    <div class="header">
        ...
        <div class="header-text">
            ...
            <p>Laporan Otomatisasi Marketing</p>
        </div>
    </div>
    ...
    <table>
        <thead>
            <tr>
                <th>No</th><th>Workflow ID</th><th>Nama</th><th>Trigger Event</th>
                <th>Syarat Tambahan</th><th>Aksi</th><th>Delay Aksi</th><th>Status</th><th>PIC</th><th>Catatan</th>
            </tr>
        </thead>
        <tbody>
            @forelse($data as $i => $d)
            <tr>
                <td>{{ $i+1 }}</td>
                <td>{{ $d->workflow_id }}</td>
                <td>{{ $d->nama_workflow }}</td>
                <td>{{ $d->trigger_event }}</td>
                <td>{{ $d->syarat_tambahan ?? '-' }}</td>
                <td>{{ $d->aksi }}</td>
                <td>{{ $d->delay_aksi ?? '-' }}</td>
                <td><span class="badge {{ $d->status==='Aktif'?'badge-green':'badge-red' }}">{{ $d->status }}</span></td>
                <td>{{ $d->pic ?? '-' }}</td>
                <td>{{ $d->catatan ?? '-' }}</td>
            </tr>
            @empty
    ...
    <div class="footer">{{ $setting?->nama_perusahaan ?? 'APY Rent' }} &mdash; Laporan Otomatisasi Marketing</div>
</body>
</html>
