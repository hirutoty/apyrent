...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Tracking UTM</h1>
            <p class="text-sm text-gray-500 mt-0.5">Pantau performa tautan UTM marketing</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('trackingutm.pdf') }}" target="_blank" class="inline-flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white text-sm font-medium px-4 py-2.5 rounded-xl shadow-sm transition-colors"><i class="fa fa-file-pdf text-sm"></i> Export PDF</a>
            <button onclick="openModal()" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2.5 rounded-xl shadow-sm transition-colors"><i class="fa fa-plus text-sm"></i> Tambah UTM</button>
        </div>
    </div>
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center text-blue-500 text-lg flex-shrink-0"><i class="fa fa-link"></i></div>
            <div><p class="text-xl font-bold text-gray-800 leading-none">{{ $total }}</p><p class="text-xs text-gray-500 mt-1">Total Tracking</p></div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl bg-purple-50 flex items-center justify-center text-purple-500 text-lg flex-shrink-0"><i class="fa fa-mouse-pointer"></i></div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl bg-yellow-50 flex items-center justify-center text-yellow-500 text-lg flex-shrink-0"><i class="fa fa-chart-line"></i></div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-gray-100">
            <div><h2 class="font-semibold text-gray-800 text-base">Daftar Tracking UTM</h2><p class="text-xs text-gray-400 mt-0.5">{{ $data->count() }} total data</p></div>
            <div class="relative"><i class="fa fa-search absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
            <input type="text" placeholder="Cari tracking..." oninput="onSearchInput(this.value)" class="pl-8 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 w-44"></div>
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100">
                        ...
                        <th class="text-left text-xs font-semibold uppercase text-gray-500 px-4 py-3">URL Tujuan</th>
                        <th class="text-left text-xs font-semibold uppercase text-gray-500 px-4 py-3">Source</th>
                        <th class="text-left text-xs font-semibold uppercase text-gray-500 px-4 py-3">Medium</th>
                        <th class="text-left text-xs font-semibold uppercase text-gray-500 px-4 py-3">Campaign</th>
                ...
                <tbody id="tableBody">
                    @forelse($data as $d)
                    <tr class="border-t border-gray-50 hover:bg-gray-50 transition-colors" data-search="{{ strtolower($d->kode_tracking.' '.$d->utm_source.' '.$d->utm_campaign.' '.$d->status) }}">
                        ...
                        <td class="px-4 py-3.5 text-xs text-gray-600">{{ $d->utm_medium }}</td>
                        <td class="px-4 py-3.5 text-xs text-gray-600">{{ $d->utm_campaign }}</td>
                        <td class="px-4 py-3.5 text-right font-semibold text-gray-800">{{ number_format($d->total_klik) }}</td>
                        <td class="px-4 py-3.5 text-right font-semibold text-green-600">{{ number_format($d->total_konversi) }}</td>
                        ...
                        <td class="px-4 py-3.5">
                            <div class="flex items-center justify-center gap-1.5">
                                <button class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-yellow-100 text-yellow-600 hover:bg-yellow-200 transition-colors"
                                    data-action="{{ route('trackingutm.update', $d->id) }}" data-kode_tracking="{{ $d->kode_tracking }}"
                                    data-url_tujuan="{{ $d->url_tujuan }}" data-utm_source="{{ $d->utm_source }}"
                                    data-utm_medium="{{ $d->utm_medium }}" data-utm_campaign="{{ $d->utm_campaign }}"
                                    data-utm_term="{{ $d->utm_term }}" data-utm_content="{{ $d->utm_content }}"
                                    data-total_klik="{{ $d->total_klik }}" data-total_konversi="{{ $d->total_konversi }}"
                                ...
                                <button type="button" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium bg-red-100 text-red-600 hover:bg-red-200 transition-colors"
                                    data-action="{{ route('trackingutm.destroy', $d->id) }}" data-name="{{ $d->kode_tracking }}" onclick="triggerDelete(this)">
                                    <i class="fa fa-trash text-xs"></i> Hapus</button>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="10" class="px-5 py-12 text-center"><p class="text-sm text-gray-400">Belum ada data tracking UTM</p></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
...
<div id="mainModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        <div class="flex items-start justify-between px-6 py-5 border-b border-gray-100 sticky top-0 bg-white">
            <h2 id="modalTitle" class="text-base font-bold text-gray-800">Tambah Tracking UTM</h2>
            <button onclick="closeModal()" class="text-gray-400 hover:text-red-500 text-lg"><i class="fa fa-times"></i></button>
        </div>
        <form id="mainForm" action="{{ route('trackingutm.store') }}" method="POST" class="px-6 py-5 space-y-4">
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30" style="backdrop-filter:blur(2px)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 px-6 py-6 text-center">
        ...
        <h2 class="text-base font-bold text-gray-800">Hapus Tracking?</h2>
        <p class="text-xs text-gray-500 mt-1.5 mb-4">Hapus kode <strong id="deleteName"></strong>?</p>
...
@endif
<script>
const mainModal=document.getElementById('mainModal'),mainForm=document.getElementById('mainForm'),methodContainer=document.getElementById('methodContainer'),createUrl="{{ route('trackingutm.store') }}";
function openModal(){document.getElementById('modalTitle').innerText='Tambah Tracking UTM';mainForm.action=createUrl;methodContainer.innerHTML='';mainForm.reset();mainModal.classList.remove('hidden');mainModal.classList.add('flex');}
...
function triggerEdit(btn){document.getElementById('modalTitle').innerText='Edit Tracking UTM';mainForm.action=btn.dataset.action;methodContainer.innerHTML='<input type="hidden" name="_method" value="PUT">';
['kode_tracking','url_tujuan','utm_source','utm_medium','utm_campaign','utm_term','utm_content','total_klik','total_konversi','status'].forEach(k=>{const el=document.getElementById('f_'+k);if(el)el.value=btn.dataset[k]??'';});
