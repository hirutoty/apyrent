...
<head>
    <meta charset="UTF-8">
    <title>Data Kampanye Marketing</title>
    <style>
...
<body>
    <div class="header">
        ...
        <div class="header-text">
            ...
            <p>Laporan Kampanye Marketing</p>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $i+1 }}</td>
                <td>{{ $d->id_kampanye }}</td>
                <td>{{ $d->nama_kampanye }}</td>
                <td>{{ $d->tipe_kampanye }}</td>
                <td>{{ $d->channel }}</td>
                <td>{{ $d->target_segment }}</td>
                <td>{{ \Carbon\Carbon::parse($d->tanggal_mulai)->format('d/m/Y') }} - {{ \Carbon\Carbon::parse($d->tanggal_akhir)->format('d/m/Y') }}</td>
                <td>{{ $d->pic ?? '-' }}</td>
                <td>
                    @php
                        $bc = match($d->status) { 'Aktif'=>'badge-green', 'Selesai'=>'badge-gray', 'Dijadwalkan'=>'badge-yellow', default=>'badge-red' };
                    @endphp
    ...
    <div class="footer">{{ $setting?->nama_perusahaan ?? 'APY Rent' }} &mdash; Laporan Kampanye Marketing</div>
</body>
</html>
