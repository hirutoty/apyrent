...
<body>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th class="text-right">Total Penjualan</th><th>% Komisi</th>
                <th class="text-right">Total Komisi</th><th>Status Bayar</th>
            </tr>
        </thead>
        <tbody>
            @forelse($data as $i => $d)
            <tr>
                ...
                <td><span class="badge badge-blue">{{ $d->persen_komisi }}%</span></td>
                <td class="text-right"><strong>Rp {{ number_format($d->total_komisi, 0, ',', '.') }}</strong></td>
                <td>
                    <span class="badge {{ ($d->status_bayar ?? 'Belum Dibayar') === 'Sudah Dibayar' ? 'badge-green' : 'badge-red' }}">
                        {{ $d->status_bayar ?? 'Belum Dibayar' }}
                    </span>
                </td>
            </tr>
            @empty
    ...
    <div class="footer">{{ $setting?->nama_perusahaan ?? 'APY Rent' }} &mdash; Laporan Komisi Sales</div>
</body>
</html>
