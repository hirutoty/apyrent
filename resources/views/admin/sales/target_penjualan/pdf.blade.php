...
<head>
    <meta charset="UTF-8">
    <title>Data Target Penjualan</title>
    <style>
...
<body>
    ...
    <table>
        <thead>
            <tr>
                <th>No</th><th>Nama Sales</th><th>Bulan</th>
                <th class="text-right">Target</th><th class="text-right">Pencapaian</th>
                <th class="text-center">% Capaian</th><th>Keterangan</th>
            </tr>
        </thead>
        <tbody>
            @forelse($data as $i => $d)
            @php
                ...
                $bc  = $pct >= 100 ? 'badge-green' : ($pct >= 70 ? 'badge-yellow' : 'badge-red');
            @endphp
            <tr>
                <td>{{ $i+1 }}</td>
                <td>{{ $d->nama_sales }}</td>
                <td>{{ $d->bulan }}</td>
                <td class="text-right">Rp {{ number_format($d->target_penjualan, 0, ',', '.') }}</td>
                <td class="text-right">Rp {{ number_format($d->pencapaian, 0, ',', '.') }}</td>
                <td class="text-center"><span class="badge {{ $bc }}">{{ $pct }}%</span></td>
                <td>{{ $d->keterangan ?? '-' }}</td>
            </tr>
            @empty
    ...
    <div class="footer">{{ $setting?->nama_perusahaan ?? 'APY Rent' }} &mdash; Laporan Target Penjualan</div>
</body>
</html>
