...
<head>
    <meta charset="UTF-8">
    <title>Data Pricelist & Diskon</title>
    <style>
...
<body>
    ...
    <table>
        <thead>
            <tr>
                <th>No</th><th>ID Harga</th><th>Nama Produk</th><th>Level Pelanggan</th>
                <th class="text-right">Harga Normal</th><th>Diskon</th>
                <th class="text-right">Harga Diskon</th><th>Periode Mulai</th><th>Periode Selesai</th><th>Status</th>
            </tr>
        </thead>
        <tbody>
            @forelse($data as $i => $d)
            <tr>
                ...
                <td>{{ $d->id_harga }}</td>
                <td>{{ $d->nama_produk }}</td>
                <td><span class="badge badge-indigo">{{ $d->level_pelanggan }}</span></td>
                <td class="text-right">Rp {{ number_format($d->harga_normal, 0, ',', '.') }}</td>
                <td><span class="badge badge-orange">{{ $d->diskon }}%</span></td>
                <td class="text-right">Rp {{ number_format($d->harga_diskon, 0, ',', '.') }}</td>
                <td>{{ \Carbon\Carbon::parse($d->periode_mulai)->format('d/m/Y') }}</td>
                <td>{{ \Carbon\Carbon::parse($d->periode_selesai)->format('d/m/Y') }}</td>
                <td><span class="badge {{ ($d->status ?? 'Aktif') === 'Aktif' ? 'badge-green' : 'badge-red' }}">{{ $d->status ?? 'Aktif' }}</span></td>
            </tr>
            @empty
    ...
    <div class="footer">{{ $setting?->nama_perusahaan ?? 'APY Rent' }} &mdash; Laporan Pricelist & Diskon</div>
</body>
</html>
