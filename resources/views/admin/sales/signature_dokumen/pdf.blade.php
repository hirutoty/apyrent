...
<head>
    <meta charset="UTF-8">
    <title>Data Signature Dokumen</title>
    <style>
...
<body>
    ...
    <table>
        <thead>
            <tr>
                <th>No</th><th>Document ID</th><th>Jenis Dokumen</th><th>Tanggal</th>
                <th>Pihak Terlibat</th><th>Platform</th><th>Status TTD</th>
            </tr>
        </thead>
        <tbody>
            @forelse($data as $i => $d)
            <tr>
                ...
                <td><span class="badge badge-indigo">{{ $d->platform_digisign }}</span></td>
                <td>
                    @php $bc = match($d->status_ttd) { 'Menunggu'=>'badge-yellow','Ditandatangani'=>'badge-green','Ditolak'=>'badge-red', default=>'badge-yellow' }; @endphp
                    <span class="badge {{ $bc }}">{{ $d->status_ttd }}</span>
                </td>
            </tr>
            @empty
    ...
    <div class="footer">{{ $setting?->nama_perusahaan ?? 'APY Rent' }} &mdash; Laporan Signature Dokumen</div>
</body>
</html>
