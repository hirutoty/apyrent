    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5">Data laporan keuangan perusahaan</p>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-1">Total Laporan Keuangan</p>
                    </div>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-xl font-bold text-green-600 mt-2 leading-tight">Rp
                            {{ number_format($data->sum('pendapatan'), 0, ',', '.') }}</h3>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-xl font-bold text-red-600 mt-2 leading-tight">Rp
                            {{ number_format($data->sum('beban'), 0, ',', '.') }}</h3>
                        <p class="text-xs text-slate-400 mt-1">Total Beban</p>
                    </div>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-slate-500">Total Laba Bersih</p>
                        <h3 class="text-xl font-bold text-purple-600 mt-2 leading-tight">Rp
                            {{ number_format($data->sum('laba'), 0, ',', '.') }}</h3>
                        <p class="text-xs text-slate-400 mt-1">Total Laba Bersih</p>
                    </div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-gray-100">
        <div>
            <h2 class="font-semibold text-gray-800 text-base">Daftar Laporan Keuangan</h2>
            <p class="text-xs text-gray-400 mt-0.5" id="totalCount">{{ $data->count() }} total data laporan</p>
        </div>
        <div class="flex items-center gap-2">
            <a id="pdfBtn"
                ...
                target="_blank" href="{{ route('konsolidasi.pdf') }}">
            ...
            <div class="relative">
                ...
                <input type="text" id="searchInput" placeholder="Cari perusahaan, periode..."
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-100">
                    ...
                    <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Nama Perusahaan</th>
                    <th class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 px-4 py-3">Pendapatan</th>
                    ...
                    <tr class="border-t border-gray-50 hover:bg-gray-50 transition-colors duration-100"
                        ...
                        <td class="px-4 py-3.5">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm font-semibold text-gray-800">{{ $item->nama_perusahaan }}</span>
                            </div>
                        </td>

                        <td class="px-4 py-3.5">
                            <span class="text-sm font-medium text-green-600">Rp {{ number_format($item->pendapatan, 0, ',', '.') }}</span>
                        </td>

                        <td class="px-4 py-3.5">
                            <span class="text-sm font-medium text-red-500">Rp {{ number_format($item->beban, 0, ',', '.') }}</span>
                        </td>

                        <td class="px-4 py-3.5">
                            <span class="text-sm font-bold text-purple-600">Rp {{ number_format($item->laba, 0, ',', '.') }}</span>
                        </td>
                        ...
                        <td class="px-4 py-3.5">
                            <div class="flex items-center justify-center gap-1.5">
                                ...
                                <form action="{{ route('konsolidasi.destroy', $item->id) }}" method="POST"
                    ...
                    <tr>
                        <td colspan="7" class="px-5 py-12 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p class="text-sm font-medium text-gray-500">Belum ada data laporan keuangan</p>
                                <p class="text-xs text-gray-400">Klik "Tambah Data" untuk menambahkan laporan baru</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
    ...
    <div id="modalTambah" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30"
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4" style="animation:slideUp .2s ease">
            ...
            <div class="flex items-start justify-between px-6 py-5 border-b border-gray-100">
                <div>
                    <h2 class="text-base font-bold text-gray-800">Tambah Laporan Keuangan</h2>
                    <p class="text-xs text-gray-500 mt-0.5">Isi data laporan keuangan perusahaan</p>
                </div>
            ...
            <form action="{{ route('konsolidasi.store') }}" method="POST" class="px-6 py-5 space-y-4">
                @csrf
...
====================================== --}}
    <div id="modalEdit" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30"
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4" style="animation:slideUp .2s ease">
            ...
            <div class="flex items-start justify-between px-6 py-5 border-b border-gray-100">
                <div>
                    <h2 class="text-base font-bold text-gray-800">Edit Laporan Keuangan</h2>
                    <p class="text-xs text-gray-500 mt-0.5">Perbarui data laporan keuangan</p>
                </div>
...
====================================== --}}
    ...
    <script>
        ...
        // ── MODAL EDIT ─────────────────────────────────────
        function openEditModal(id, nama, pendapatan, beban, periode) {
            var m = document.getElementById('modalEdit');
            m.classList.remove('hidden');
            m.classList.add('flex');

            document.getElementById('formEdit').action = '/admin/konsolidasi/' + id;
            document.getElementById('edit_nama_perusahaan').value = nama;
            document.getElementById('edit_pendapatan').value = pendapatan;
            document.getElementById('edit_beban').value = beban;
        ...
        function updateExportLinks() {
            ...
            document.getElementById('pdfBtn').href = `/admin/konsolidasi/pdf${query}`;
            document.getElementById('excelBtn').href = `/admin/konsolidasi/excel${query}`;
        }
...
function filterTable(q) {
    ...
    // update counter
    document.getElementById('totalCount').textContent = visible + ' total data laporan';
